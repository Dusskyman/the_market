...
class SaveUserToStoreEpic extends BaseEpic<SaveUserToStoreAction> {
  @override
  Stream epic(SaveUserToStoreAction action, EpicStore<AppState> store) async* {
    try {
      await UserStorage.instance().saveUser(action.userDto);
    } catch (error) {
      Log.e('SaveUserToStoreEpic error: ', error);
    }
  }
}
